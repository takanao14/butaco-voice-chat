...
func TestFetchAndSummarize(t *testing.T) {
	...
	for _, want := range []string{
		"今日は 9月25日（金）。",
		"■アーセナル（ユーザーが応援）",
		"直近の試合: 9月19日（土）23時ちょうど（6日前）プレミアリーグ、アウェイでブライトンと対戦し、0-3 で負け。",
		...
		"プレミアリーグ順位: 2位、勝点18",
		"直近の試合: 9月24日（木）21時ちょうど（昨日）プレミアリーグ、ホームでフラムと対戦し、2-1 で勝ち。",
		"現在試合中: 9月25日（金）16時ちょうど（今日）リーグカップ、ホームでエヴァートンと対戦中。途中経過は分からない。",
		"次の試合: 情報なし。",
		"今日の試合: なし。",
		"今日の試合: あり（9月25日（金）16時ちょうど（今日）リーグカップ、ホームでエヴァートンと対戦）。",
...
func TestFormatTime(t *testing.T) {
	for utc, want := range map[string]string{
		"2026-09-25T09:00:00Z": "9月25日（金）18時ちょうど",
		"2026-09-25T09:05:00Z": "9月25日（金）18時5分",
		"2026-10-10T11:30:00Z": "10月10日（土）20時30分",
	} {
		at, _ := time.Parse(time.RFC3339, utc)
		if got := formatTime(at); got != want {
			t.Errorf("formatTime(%s) = %q, want %q", utc, got, want)
		}
	}
}
